...
- (void)keyboardDidShow:(NSNotification *)notification
{
    ...
    CGFloat kbHeight = kbSize.height /*+ self.inputAccessoryView.frame.size.height + self.adjustInset*/;
    ...
    switch ( self.forceBehaviour )
    {
        ...
        case ForceBehaviourPositon:
            if ( [self.forceObject isKindOfClass:[NSValue class]] )
            {
                [self.scrollView setContentOffset:[self.forceObject CGPointValue] animated:YES];
            }
            else
            {
                @throw [NSException exceptionWithName:@"WrongClassError" reason:@"Expecting NSValue for forceObject" userInfo:nil];
            }
            break;
        ...
        case ForceBehaviourCentreOnView:
            if ( [self.forceObject isKindOfClass:[UIView class]] )
            {
                [self _scrollToCentreOfArea:[self.scrollView convertRect:[self.forceObject bounds] fromView:self.forceObject]];
            }
            else
            {
                @throw [NSException exceptionWithName:@"WrongClassError" reason:@"Expecting UIView for forceObject" userInfo:nil];
            }
    }
}
...
- (void)_updateScrollOffset
{
    ...
    {
        @throw [NSException exceptionWithName:@"KeyboardNilActiveViewError" reason:@"the keyboard cannot be show with no active viwe" userInfo:nil];
    }
#endif
    
    // if the active field is hidden, move it up
    UIView * view = self.scrollView;
    CGRect r = [self _viewableBounds];
...
{
    CGRect viewableBounds = [self _viewableBounds];
